...
import { MatchersV3, PactV3, type Rules } from '@pact-foundation/pact';
  ...
  it('uploads multipart form with JSON metadata and image using matching rules', async () => {
    ...
    const requestMatchingRules: Rules = {
      body: [
        {
          path: '$.image',
          rules: [MatchersV3.contentType('image/jpeg')],
        },
      ],
      header: [
        {
    ...
    try {
      await pact
        ...
        .executeTest(async (mockServer) => {
          ...
          expect(response.data.id).toBe('upload-1');
          expect(response.data.metadata).toEqual({
            name: 'test',
            size: 100,
          });
          expect(response.data.image_size).toBe(JPEG_BYTES.length);
        });
    } finally {
