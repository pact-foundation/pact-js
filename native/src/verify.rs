...
fn consumer_tags_to_selectors(tags: Vec<String>) -> Vec<pact_verifier::ConsumerVersionSelector> {
    tags.iter()
        .map(|t| pact_verifier::ConsumerVersionSelector {
            consumer: None,
            fallback_tag: None,
            tag: t.to_string(),
            latest: Some(true),
...
pub fn verify_provider(mut cx: FunctionContext) -> JsResult<JsUndefined> {
        ...
        Ok(urls) => {
            match urls.downcast::<JsArray>() {
              ...
              _ => println!("    {}", Yellow.paint ("WARN: pactUrls does not contain a valid list of URL strings"))
                ...
                _ => {
                    if !urls.is_a::<JsUndefined>() && !urls.is_a::<JsNull>() {
                        println!(
                            "    {}",
                            Yellow.paint("WARN: pactUrls is not a list of URLs, ignoring")
                        );
                    }
                }
            }
    ...
    match config.get(&mut cx, "pactBrokerUrl") {
        Ok(url) => match url.downcast::<JsString>() {
            Ok(url) => {
                let enable_pending = get_bool_value(&mut cx, &config, "enablePending");
                ...
                if let Some(username) = get_string_value(&mut cx, &config, "pactBrokerUsername") {
                    ...
                    pacts.push(PactSource::BrokerWithDynamicConfiguration {
                        ...
                        broker_url: url.value(),
                        enable_pending,
                        include_wip_pacts_since: wip,
                        provider_tags: provider_tags.clone(),
                        selectors,
                        auth: Some(HttpAuth::User(username, password)),
                        links: vec![],
                    })
            ...
            Err(_) => {
                if !url.is_a::<JsUndefined>() {
                    println!(
                        "    {}",
                        Red.paint("ERROR: pactBrokerUrl must be a string value")
                    );
    ...
    match get_string_value(&mut cx, &config, "providerBaseUrl") {
        Some(url) => match Url::parse(&url) {
            ...
            Err(err) => {
                ...
                println!(
                    "    {}",
                    Red.paint("ERROR: pactBrokerUrl is not a valid URL")
                );
            }
        },
        None => (),
    ...
    let provider_version = match config.get(&mut cx, "providerVersion") {
        Ok(provider_version) => match provider_version.downcast::<JsString>() {
            ...
            Err(_) => {
                if !provider_version.is_a::<JsUndefined>() {
                    println!(
                        "    {}",
                        Red.paint("ERROR: providerVersion must be a string value")
                    );
